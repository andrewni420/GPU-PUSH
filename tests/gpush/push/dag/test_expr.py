from gpush.push.dag.expr import Function, Constant, Input
...
def make_tree():
    ...
    # Leaves
    c0,c1,c2,c3,c4 = constants = [Constant(i,param_idx=i) for i in range(5)]
    i0,i1,i2,i3,i4 = inputs =  [Input(i+5, input_idx=i) for i in range(5)]

    # Functions of leaves
    f0 = Function(id = 10, fn = identity, children = (c0, c4, i2))
    f0_ = Function(id = 12, fn = identity, children = (c0, c4, i2))
    f1 = Function(id = 14, fn = identity, children = tuple())
    f1_ = Function(id = 15, fn = identity, children = dict())
    f2 = Function(id = 17, fn = identity, children = (c1,))
    f3 = Function(id = 19, fn = identity, children = (c0, i0))
    ...
    f10 = Function(id = 50, fn = identity, children = (f1,c1,i4))
    ...
    # Constant outputs
    oc0,oc1,oc2,oc3,oc4 = oconstants = params 
    oi0,oi1,oi2,oi3,oi4 = oinputs = input 

    # Function outputs
    of0 = ((oc0,oc4,oi2), dict())
    of0_ = ((oc0,oc4,oi2), dict())
    of1 = (tuple(), dict())
    of1_ = (tuple(), dict())
    of2 = ((oc1,), dict())
    of3 = ((oc0, oi0), dict())
    ...
    return (root, constants, inputs, functions, _functions), (params, input, (oroot, oconstants, oinputs, ofunctions, _ofunctions))

def test_gather():
    "Test whether `gather()` correctly returns the nodes in the tree"
    (tree, constants, inputs, functions, _functions), _ = make_tree()
    expressions = tree.gather()
    target_expressions = [constants[0], constants[1], constants[4], inputs[0], inputs[2], inputs[4]] + functions 
...
def test_initial_eval():
    ...
    (tree, constants, inputs, functions, _functions), (params, input, _) = make_tree()
    cache = [None]*len(tree.gather())
    tree.normalize()

    # Test constants
    for c,p in zip(constants,params):
        assert c.eval(params,input,cache)==p 
    # Test inputs
