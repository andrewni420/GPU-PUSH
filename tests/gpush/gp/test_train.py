...
def test_lambda_schedule(args, kwargs):
    ...
    s = LambdaSchedule(identity,args,kwargs)
    assert s.initial_value()==(0,None,args,kwargs)
...
def test_stepwise_linear_schedule(steps, step, cur_val):
    s = StepwiseLinearSchedule(steps)
    if step>=0:
        if len(steps)==0:
            res = cur_val
        elif step<steps[0][0]:
        ...
        assert res==s.step(step,cur_val)
...
@pytest.mark.parametrize("params,grad",[([jnp.zeros((1,)), jnp.ones((2,3)), jnp.array([[1000.1,2.2,3],[4,1,2.4],[1,205.2,2]])], [jnp.ones((1,)), jnp.zeros((2,3)), jnp.array([[0.4,1.3,2.4],[4,1.2,4],[100.5,-204.1,0.00001]])]), 
                                        ...
                                        ([],[])])
@pytest.mark.parametrize("lr", [ConstantSchedule(1.),StepwiseLinearSchedule(((0,1.),(100,0.001))), CosineSchedule(0.1,100), LambdaSchedule(lambda i,cur_val,arg1,kwarg1=0:(cur_val or 1.)*i+(arg1/kwarg1), (10.,),{"kwarg1":-9.})])
@pytest.mark.parametrize("step", [0,1,10,100])
def test_sgd(params, grad, lr, step):
    res = [p-lr.step(step,lr.initial_value())*g for p,g in zip(params,grad)]
    sgd = SGD(lr)
    ...
    assert state == {"params":params, "lr":lr.initial_value()} 
    assert isinstance(state["lr"],float) or jnp.ndim(state["lr"])==0
    assert state2["lr"] == lr.step(step,lr.initial_value())
...
@pytest.mark.parametrize("lr", [ConstantSchedule(0.1),StepwiseLinearSchedule(((0,0.1),(100,0))), CosineSchedule(0.1,100), LambdaSchedule(lambda i,cur_val,arg1,kwarg1=0:0.1 if i==0 else (cur_val/kwarg1 if i==arg1 else cur_val), (40,),{"kwarg1":10})])
...
@pytest.mark.parametrize("params,velocity,grad",[([jnp.zeros((1,)), jnp.ones((2,3)), jnp.array([[1000.1,2.2,3],[4,1,2.4],[1,205.2,2]])], [jnp.ones((1,)), jnp.zeros((2,3)), jnp.array([[-20.1,22.2,203],[-4,1.001,-2.4],[1.91,-25.2,-4]])], [jnp.zeros((1,)), jnp.ones((2,3)), jnp.array([[0.4,1.3,2.4],[4,1.2,4],[100.5,-204.1,0.00001]])]), 
                                        ...
                                        ([],[],[])])
@pytest.mark.parametrize("lr", [ConstantSchedule(1.),StepwiseLinearSchedule(((0,1.),(100,0.001))), CosineSchedule(0.1,100), LambdaSchedule(lambda i,cur_val,arg1,kwarg1=0:(cur_val or 1.)*i+(arg1/kwarg1), (10.,),{"kwarg1":-9.})])
@pytest.mark.parametrize("mass", [ConstantSchedule(0.9),StepwiseLinearSchedule(((0,0.99),(100,0.5)))])
@pytest.mark.parametrize("step", [0,1,10,100])
def test_momentum(params, velocity, grad, lr, mass, step):
    vel = [v*mass.step(step,mass.initial_value())+g for v,g in zip(velocity,grad)]
    res = [x-lr.step(step,lr.initial_value())*v for x,v in zip(params,vel)]
    ...
    assert all([jnp.allclose(rp,r) for rp,r in zip(res_params,res)])
    assert state == {"params":params, "lr":lr.initial_value(), "momentum":velocity, "mass": mass.initial_value()} 
    assert isinstance(state["lr"],float) or jnp.ndim(state["lr"])==0
    assert isinstance(state["mass"],float) or jnp.ndim(state["mass"])==0
    assert state2["lr"] == lr.step(step,lr.initial_value())
    assert state2["mass"] == mass.step(step,mass.initial_value())
    assert isinstance(state2["lr"],float) or jnp.ndim(state2["lr"])==0
...
@pytest.mark.parametrize("params,velocity,var,grad",[([jnp.zeros((1,)), jnp.ones((2,3)), jnp.array([[1000.1,2.2,3],[4,1,2.4],[1,205.2,2]])], [jnp.ones((1,)), jnp.zeros((2,3)), jnp.array([[-20.1,22.2,203],[-4,1.001,-2.4],[1.91,-25.2,-4]])], [jnp.zeros((1,)), jnp.ones((2,3)), jnp.array([[1.4,10.2,2.03],[4,20.001,2.4],[19.1,2.223,10.2]])], [jnp.zeros((1,)), jnp.ones((2,3)), jnp.array([[0.4,1.3,2.4],[4,1.2,4],[100.5,-204.1,0.00001]])]), 
                                        ...
                                        ([],[],[],[])])
@pytest.mark.parametrize("lr", [ConstantSchedule(1.),StepwiseLinearSchedule(((0,1.),(100,0.001))), CosineSchedule(0.1,100), LambdaSchedule(lambda i,cur_val,arg1,kwarg1=0:(cur_val or 1.)*i+(arg1/kwarg1), (10.,),{"kwarg1":-9.})])
@pytest.mark.parametrize("b1", [ConstantSchedule(0.9),StepwiseLinearSchedule(((0,0.99),(100,0.5)))])
@pytest.mark.parametrize("b2", [ConstantSchedule(0.99),StepwiseLinearSchedule(((0,0.999),(100,0.9)))])
...
def test_adam(params, velocity,var,grad, lr, b1,b2,eps,step):
    b1_ = b1.step(step,b1.initial_value())
    b2_ = b2.step(step,b2.initial_value())
    lr_ = lr.step(step,lr.initial_value())
    eps_ = eps.step(step,eps.initial_value())

    velocity_ = [((1-b1_)*g+b1_*v)/(1-jnp.asarray(b1_)**(step+1)) for g,v in zip(grad,velocity)]
    var_ = [((1-b2_)*jnp.square(g))/(1-jnp.asarray(b2_)**(step+1)) + b2_*v for g,v in zip(grad,var)]
    res = [x-lr_*v/(jnp.sqrt(v_)+eps_) for x,v,v_ in zip(params,velocity_,var_)]
    ...
    assert all([jnp.allclose(rp,r) for rp,r in zip(res_params,res)])
    assert state == {"params":params, "velocity":velocity, "var":var, "lr":lr.initial_value(), "b1":b1.initial_value(),"b2":b2.initial_value(),"eps":eps.initial_value()} 
    ...
    for k in ["lr", "b1","b2","eps"]:
        ...
        assert state2[k] == hyperparams[k].step(step,hyperparams[k].initial_value())
